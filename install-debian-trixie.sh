...
# Installer for the framebuffer extension on Debian Trixie
...
# - Installs fb.so and enables it for CLI, FPM, and Apache SAPIs
...
echo "=========================================="
echo " Framebuffer Extension Installer (Debian Trixie / Raspberry Pi OS)"
echo "=========================================="
echo ""
...
for attempt in 1 2 3 4 5; do
    if "$PHP_BIN_REAL" -r 'exit(class_exists("Fb\\Buffer", false) ? 0 : 1);' 2>/dev/null; then
        ok "Fb\\Buffer is available in CLI"
        VERIFIED=1
        break
    fi
    sleep 1
...
if [ "$VERIFIED" -eq 0 ]; then
    ...
    die "Fb\\\\Buffer not available after install. Check ${INI_NAME} and build.log."
fi

if ! "$PHP_BIN_REAL" -m 2>/dev/null | grep -qx "${EXTENSION_NAME}"; then
    echo "   ⚠️  ${EXTENSION_NAME} not listed in php -m (Fb\\\\Buffer is loaded — may be OK)"
else
